...
typedef struct {
    ...
    uint8_t rx_last_sn;
    uint8_t tx_sn;
} agcore_data_link_state_t;

static agcore_data_link_state_t g_agcore_data_link_state[LINK_COUNT];

/**
 * @brief 获取指定链路的连接状态
 * @param link 链路
 * @return 链路状态指针, 无效链路返回 NULL
 */
static agcore_data_link_state_t *agcore_data_link_state_get(agcore_link link)
{
    if (link <= LINK_NONE || link >= LINK_COUNT) {
        return NULL;
    }

    return &g_agcore_data_link_state[link];
}
...
{
    agcore_data_link_state_t *state = agcore_data_link_state_get(link);
    if (state == NULL) {

...
static display_st7735_runtime_t g_runtime = {
    .width = CONFIG_DISPLAY_ST7735_WIDTH,
    .height = CONFIG_DISPLAY_ST7735_HEIGHT,
    .rotation = DISPLAY_ST7735_ROTATION_0,
};
 ...
 * @brief SPI 发送完成会回调
 */
static bool st7735_color_trans_done(esp_lcd_panel_io_handle_t panel_io, esp_lcd_panel_io_event_data_t *edata, void *user_ctx)
{
    (void)panel_io;
    (void)edata;
    (void)user_ctx;

    if (g_runtime.flush_done_cb != NULL) {
        g_runtime.flush_done_cb();
    }
    return false;
}
...
{
    esp_err_t ret = esp_lcd_panel_io_tx_param(g_runtime.panel_io, command, data, size);
    if (ret != ESP_OK) {
 ...
 * @return esp_err_t
 */
esp_err_t display_st7735_set_rotation(display_st7735_rotation rotation)
{
    ...
    if (ret == ESP_OK) {
        g_runtime.rotation = rotation;
        if (rotation == DISPLAY_ST7735_ROTATION_90 || rotation == DISPLAY_ST7735_ROTATION_270) {
            g_runtime.width = CONFIG_DISPLAY_ST7735_HEIGHT;
            g_runtime.height = CONFIG_DISPLAY_ST7735_WIDTH;
        } else {
            g_runtime.width = CONFIG_DISPLAY_ST7735_WIDTH;
            g_runtime.height = CONFIG_DISPLAY_ST7735_HEIGHT;
        }
        CORE_LOGD(TAG, "rotation=%d madctl=0x%02X size=%ux%u", rotation, madctl, g_runtime.width, g_runtime.height);
    }
    return ret;
}
...
/**
 * @brief 获取运行时方向
 * @return display_st7735_rotation
 */
display_st7735_rotation display_st7735_get_rotation(void)
{
    return g_runtime.rotation;
}
...
esp_err_t display_st7735_flush(uint16_t x_start, uint16_t y_start, uint16_t x_end, uint16_t y_end, const void *rgb565_data)
{
    ...
    if (rgb565_data == NULL || x_start >= x_end || y_start >= y_end ||
            x_end > display_st7735_get_width() || y_end > display_st7735_get_height()) {
    ...
    ret = esp_lcd_panel_io_tx_color(g_runtime.panel_io, LCD_CMD_RAMWR, rgb565_data, (size_t)draw_w * draw_h * sizeof(uint16_t));
...
{
    if (g_runtime.test_task_handle != NULL) {
        return ESP_ERR_INVALID_STATE;
    }
    ...
    BaseType_t task_result = xTaskCreate(st7735_test_task, "st7735_test", (3 * 1024),  frame_buffer,  5, &g_runtime.test_task_handle);
    if (task_result != pdPASS) {
        heap_caps_free(frame_buffer);
        g_runtime.test_task_handle = NULL;
...
esp_err_t display_st7735_init(display_st7735_flush_done_cb_t cb)
{
    ...
    g_runtime.width = CONFIG_DISPLAY_ST7735_WIDTH;
    g_runtime.height = CONFIG_DISPLAY_ST7735_HEIGHT;
    g_runtime.rotation = DISPLAY_ST7735_ROTATION_0;

...
static SemaphoreHandle_t g_console_tx_lock;   /* 与 esh 共享的串口写锁 */
static StaticSemaphore_t g_console_lock_buf;
...
{
    g_console_tx_lock = xSemaphoreCreateMutexStatic(&g_console_lock_buf);
    if (!g_console_tx_lock) {
        return;
    }
    agcore_log_init();
}
...
{
    xSemaphoreTake(g_console_tx_lock, portMAX_DELAY);
}
...
{
    xSemaphoreGive(g_console_tx_lock);
}

...
static agcore_ble_adv_data_t g_agcore_ble_adv_data;
static const struct ble_gap_adv_params g_agcore_ble_adv_params = {
...
{
    int ret = ble_gap_adv_start(agcore_ble_get_addr_type(), NULL, BLE_HS_FOREVER, &g_agcore_ble_adv_params, agcore_ble_gap_event_cb, NULL);
    if (ret) {
...
{
    memset(&g_agcore_ble_adv_data, 0, sizeof(g_agcore_ble_adv_data));
}

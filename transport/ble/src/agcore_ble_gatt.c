...
static const ble_uuid16_t g_agcore_ble_chr_control_notify   = BLE_UUID16_INIT(0x4302);
 ...
 * @brief GATT服务表
 */
static const struct ble_gatt_svc_def g_agcore_ble_gatt_svcs[] = {
    /*** Service: CONTROL. */
    {
        .type = BLE_GATT_SVC_TYPE_PRIMARY,
        .uuid = &g_agcore_ble_svc_control.u,
        .characteristics = (struct ble_gatt_chr_def[])
        {
            {
                .uuid = &g_agcore_ble_chr_control_write.u,
                .access_cb = agcore_ble_gatt_write_cb,
                .flags = BLE_GATT_CHR_F_WRITE | BLE_GATT_CHR_F_WRITE_NO_RSP,
            }, {
                .uuid = &g_agcore_ble_chr_control_notify.u,
...
void agcore_ble_gatt_init(void)
{
    ...
    ret = ble_gatts_count_cfg(g_agcore_ble_gatt_svcs);
    if (ret != 0) {
        CORE_LOGE(TAG, "count service config failed|%d", ret);
        return;
    }

    ret = ble_gatts_add_svcs(g_agcore_ble_gatt_svcs);
    if (ret != 0) {

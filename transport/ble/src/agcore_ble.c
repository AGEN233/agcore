...
static TaskHandle_t g_agcore_ble_host_handle = NULL;
static uint8_t g_agcore_ble_addr_type = 0;
static atomic_bool g_agcore_ble_ready = false;
...
{
    return atomic_load(&g_agcore_ble_ready);
}
...
/**
 * @brief 协议栈与控制器完成同步回调: 初始化 GAP/GATT/广播并标记就绪
 */
static void agcore_ble_on_stack_sync_cb(void)
{
    CORE_LOGD(TAG, "host sync success");

    int ret = ble_hs_id_infer_auto(0, &g_agcore_ble_addr_type);
    if (ret != 0) {
    ...
    uint8_t mac[6];
    ret = ble_hs_id_copy_addr(g_agcore_ble_addr_type, mac, NULL);
    if (ret != 0) {
    ...
    atomic_store(&g_agcore_ble_ready, true);
}
...
static esp_err_t agcore_ble_nimble_init(void)
{
    ...
    if (g_agcore_ble_host_handle == NULL) {
        xTaskCreate(agcore_ble_host_task, "AGBLEHOST", 4 * 1024, NULL, 5, &g_agcore_ble_host_handle);
        if (g_agcore_ble_host_handle == NULL) {

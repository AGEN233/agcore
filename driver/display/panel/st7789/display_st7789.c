...
static display_st7789_runtime_t g_runtime = {
...
esp_err_t display_st7789_set_rotation(display_st7789_rotation rotation)
{
    ...
    if (ret == ESP_OK) {
        ...
        if (rotation == DISPLAY_ST7789_ROTATION_90 || rotation == DISPLAY_ST7789_ROTATION_270) {
            g_runtime.width = CONFIG_DISPLAY_ST7789_HEIGHT;
            g_runtime.height = CONFIG_DISPLAY_ST7789_WIDTH;
        } else {
            g_runtime.width = CONFIG_DISPLAY_ST7789_WIDTH;
            g_runtime.height = CONFIG_DISPLAY_ST7789_HEIGHT;
        }
...
/**
 * @brief 获取当前运行时方向
 * @return display_st7789_rotation
 */
...
esp_err_t display_st7789_flush(uint16_t x_start, uint16_t y_start, uint16_t x_end, uint16_t y_end, const void *rgb565_data)
{
    ...
    if (g_runtime.rotation == DISPLAY_ST7789_ROTATION_90 ||
            g_runtime.rotation == DISPLAY_ST7789_ROTATION_270) {
...
esp_err_t display_st7789_test(void)
{
    ...
    BaseType_t task_result = xTaskCreate(st7789_test_task, "st7789_test", (3 * 1024), frame_buffer, 5, &g_runtime.test_task_handle);
...
esp_err_t display_st7789_init(display_st7789_flush_done_cb_t cb)
{
    ...
    ret = esp_lcd_new_panel_io_spi((esp_lcd_spi_bus_handle_t)ST7789_SPI_HOST, &io_config, &g_runtime.panel_io);
    if (ret != ESP_OK) {
    ...
    g_runtime.width = CONFIG_DISPLAY_ST7789_WIDTH;
    g_runtime.height = CONFIG_DISPLAY_ST7789_HEIGHT;
    g_runtime.rotation = DISPLAY_ST7789_ROTATION_0;
